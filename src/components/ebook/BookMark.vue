<template>
  <div class="ebook-bookmark" ref="ebookBookmark">
    ...
    <div
      ...
      :style="fixed && !isPaginating ? fixedStyle : {}"
    >
      <book-mark-icon
        :width="15"
        :height="35"
...
export default {
  ...
  computed: {
    ...
    fixedStyle() {
      return {
        position: "fixed",
        right: `${
          (window.innerWidth - this.$refs.ebookBookmark.clientWidth) / 2
        }px`,
        top: 0,
      };
    },
  ...
  watch: {
    offsetY(v) {
      console.log(this.settingVisible, this.menuVisible, this.isPaginating);
